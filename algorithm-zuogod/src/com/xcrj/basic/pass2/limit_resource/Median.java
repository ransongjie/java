 ...
 * 40亿个无符号整数，中位数
 * v3: 
 * - 中位数是第20亿和第20亿+1的数》数值分片统计，数值a到b范围内的数字的个数》累计每个分片数字个数
 * - 若第m个分片包含第20亿和第20亿+1的数》对这一分片重复上述过程
 * - 分片大小：
 * -- 整数数量》这些整数都是同1个数》分片最大值》分片大小》分片数据类型
 * -- 一个分片用于记录a~b范围内数字的个数，40亿个数，40亿个数都是同1个数，分片最大值40亿，4B的uint值够存
 * - 分片数量：提供内存大小10MB / 记录项大小4B(uint类型)
 * - 分片范围：数值范围/分片数量
 * 
 * v2
 * - 数值分片
 ...
 * 桶数值分片统计：在10MB内存下，将2^32数值区间划分成250万个小数值区间(桶数量)，每个小区间大小为1680(桶区间)
 * 
